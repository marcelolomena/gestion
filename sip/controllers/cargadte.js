...
exports.detalle = function (req, res) {
  logger.debug("en detalle:" + req.params.id);
  var page = req.body.page;
  var rows = req.body.rows;
  var filters = req.body.filters;
  var sidx = req.body.sidx;
...
exports.archivo = function (req, res) {
  ...
  if (req.method === 'POST') {
    ...
    var existPref = function (prefactura) {
        ...
        try {
          ...
          return sequelize.query(sql1)
            .spread(function (rows) {
              if (rows.length > 0) {
                logger.debug("Existe prefactura:" + rows[0].id);
                resolve(1);
              } else {
                resolve(0);
              }
    ...
    var montoEnRango = function (monto, prefactura) {
      return new Promise(function (resolve, reject) {
        try {
          ...
          return sequelize.query(sql1)
            .spread(function (rows) {
              if (rows.length > 0) {
                logger.debug("Factor prefactura:" + rows[0].valor);
                var factor = rows[0].valor;
                return sequelize.query(sql2).spread(function (rows2) {
                  var totalpref = rows2[0].totalprefactura;
                  var tolerancia = parseFloat(totalpref) * parseFloat(factor);
                  logger.debug("Tolerancia:" + tolerancia + " monto:" + totalpref + " Factor:" + factor);
                  var montomin = parseFloat(totalpref) - parseFloat(tolerancia);
                  var montomax = parseFloat(totalpref) + parseFloat(tolerancia);
                  logger.debug("Comparando:" + monto + " entre:" + montomin + " y " + montomax);
                  if (parseFloat(monto) >= parseFloat(montomin) && parseFloat(monto) <= parseFloat(montomax)) {
                    logger.debug("MOnto OK:");
                    resolve(1);
                  } else {
                    logger.debug("MOnto NOK:");
                    resolve(0);//Monto fuera de rango
                  }
                }).catch(function (err) {
                  logger.error(err)
                  resolve(-1);//Monto fuera de rango
                });
              } else {
                logger.debug('No esta factor de tolerancia');
                resolve(-1);
              }
            }).catch(function (err) {
              logger.error(err);
    ...
    var creaProcesoDTE = function (idcargadte, zipEntryName, estadomatch, factura) {
      return new Promise(function (resolve, reject) {
        try {
          logger.debug("Grabando Detalle " + idcargadte + ", " + estadomatch);
          if (factura != -1) {
            ...
            return models.proveedor.findOne({
              ...
              if (proveedor) {
                ...
                return models.factura.create({
                  ...
                  existPref(IdPrefactura).then(function (prefactura) {
                    logger.debug("Existe prefactura:" + prefactura);
                    if (prefactura == 1) {
                      montoEnRango(MntTotal, IdPrefactura).then(function (monto) {
                        logger.debug("Match Monto:" + monto);
                        if (monto == 1) {
                          estadomatch = "EXITO";
                        } else if (monto = 0) {
                        ...
                        creaProcesoDTE(idcargadte, zipEntryName, estadomatch, factura.id).then(function (id) {
                          logger.debug("Grabo detalle 1 con " + id);
                        });
                      });
                    } else if (prefactura == 0) {
                      estadomatch = "NO EXISTE PREFACTURA";
                      creaProcesoDTE(idcargadte, zipEntryName, estadomatch, factura.id).then(function (id) {
                        logger.debug("Grabo detalle 2 con " + id);
                      });
                    } else {
                      estadomatch = "ERROR PREFACTURA";
                      creaProcesoDTE(idcargadte, zipEntryName, estadomatch, factura.id).then(function (id) {
                        logger.debug("Grabo detalle 3 con " + id);
                      });
                    }
                  });
                  logger.debug("Glosa obtenida:" + estadomatch);

                  return factura.id;

                }).catch(function (err) {
                  logger.debug("Error al crear factura:" + err)
                  throw new Error(err);
                });
              } else {
                creaProcesoDTE(idcargadte, zipEntryName, "NO Existe Proveedor", -1, function (prefactura) {
                  logger.debug("Grabo detalle sin proveedor");
                });
              }

            }).catch(function (err) {
              logger.debug("Error al buscar RUT:" + err)
              throw new Error(err);
            });
        ...
        try {
          ...
          return models.sequelize.transaction({ autocommit: true }, function (t) {
            ...
            if (ini > -1) {
              ...
              logger.debug("IDPREFACTURADTE : " + results[0] + " ID Fact:" + results[1]);
    ...
    var processZipEntries = function (zipEntries, zipFolderName, idcarga) {

      function recursiveCall(zipEntries, zipFolderName, i, idcarga, resolve, reject) {
        ...
        if (i < zipEntries.length) {
          ...
          logger.debug("Prefactura Item 1 : " + results[0]);
          ...
          if (results[0] != undefined) {//encuentra numero de prefactura   
            ...
            recursiveCall(zipEntries, zipFolderName, i + 1, idcarga, resolve, reject);

          } else {
            return creaProcesoDTE(idcarga, zipEntryName, "NO Contiene Id Prefactura", -1, function (prefactura) {
              logger.debug("Grabo detalle sin prefactura:" + idcarga + ", " + zipEntryName);
            });
            recursiveCall(zipEntries, zipFolderName, i + 1, idcarga, resolve, reject);
      ...
      return new Promise(function (resolve, reject) {
        recursiveCall(zipEntries, zipFolderName, 0, idcarga, resolve, reject);
      });
    ...
    var delCarga = function (idcarga) {
      return new Promise(function (resolve, reject) {
        try {
          var sql1 = "DELETE FROM sip.cargadte WHERE id=" + idcarga
          return sequelize.query(sql1)
            .spread(function (rows) {
              resolve(0); //No existe prefactura
            }).catch(function (err) {
              logger.error(err);
              resolve(-1);
            });
      ...
      file.on('end', function () {
        ...
        awaitId.then(function (idcargadte) {
          try {
            ...
            logger.debug("Filename" + zipFolderName)

            try {

              processZipEntries(zipEntries, zipFolderName, idcargadte).then(function (lastid) {
                logger.debug("FIN DE LA HISTORIA " + lastid)

                return models.procesodte.count({
                  where: { glosa: 'EXITO', idcarga: idcargadte }
                }).then(function (exito) {
                  logger.debug("LALALA : " + exito)
                ...
                /*
                                var sql1 = "DECLARE @exito INT;" +
                                  ...
                                  "DECLARE @error INT; " +
                                  "SELECT @exito=count(*) FROM sip.procesodte WHERE glosa = 'EXITO'  AND idcarga=" + idcargadte + " " +
                                  "SELECT @error=count(*) FROM sip.procesodte WHERE NOT glosa = 'EXITO' AND idcarga=" + idcargadte + " " +
                                  "IF (@error > 0) " +
                                  "BEGIN " +
                                  ...
                                  "END " +
                                  "UPDATE sip.cargadte SET exito=@exito, error=@error, estado=@msg  WHERE  id=" + idcargadte;
                                return sequelize.query(sql1)
                                  .spread(function (rows) {
                                    logger.debug("LALALA : " + rows)
                                    return res.json({ message: "archivo " + filename + " cargado", success: true });
                                  }).catch(function (err) {
                                    logger.error(err);
                                    return res.json({ message: "archivo WWW " + filename + " cargado", success: false });
                                  });
                */
...
*/
              ...
              logger.debug("Actualiza cargadte S/E");
              /*
return models.cargadte.update({
  horafin: new Date(),
  archivo: filename,
  estado: data
    ...
    return res.json({ message: err, success: false });
                  ...
                  logger.debug("Actualiza cargadte S/E"+err);
                  return models.cargadte.update({
                    horafin: new Date(),
                    archivo: filename,
                    estado: err
...
*/
            ...
            return delCarga(idcargadte).then(function (ret) {
              logger.debug("salio del borrado " + ret)
...
exports.updateResumen = function (req, res) {
  logger.debug("*********************Inicia Resumen cargadte:" + req.params.id);
