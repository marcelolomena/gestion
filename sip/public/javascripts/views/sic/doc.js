...
function gridabrobaciondoc(parentRowID, parentRowKey, suffix) {
    ...
    var tmpla = "<div id='responsive-form' class='clearfix'>";


    tmpla += "<div class='form-row'>";
    tmpla += "<div class='column-full'>Tipo<span style='color:red'>*</span>{tipo}</div>";
    tmpla += "</div>";

    tmpla += "<div class='form-row'>";
    tmpla += "<div class='column-full'>Observacion<span style='color:red'>*</span>{observacion}</div>";
    tmpla += "</div>";

    tmpla += "<hr style='width:100%;'/>";
    tmpla += "<div> {sData} {cData}  </div>";
    tmpla += "</div>";
    ...
    var parentSolicitud = rowData.id;
    console.log("la parentSolicitud : " + parentSolicitud)

    var childGridURL = "/sic/listaaprobaciondocumento/" + parentSolicitud + "/list";
    
    $("#" + childGridID).jqGrid({
        url: childGridURL,
        datatype: "json",
        mtype: "GET",
        colNames: ['id', 'iddocumentacioncotizacion', 'Nombre', 'Apellido', 'Tipo', 'Observación'],
        colModel: [
            { name: 'id', index: 'id', key: true, hidden: true },
            { name: 'iddocumentacioncotizacion', index: 'iddocumentacioncotizacion', hidden: true },
            ...
            {
                name: 'observacion', index: 'observacion', editable: true,
    ...
    $("#" + childGridID).jqGrid('navGrid',  "#" + childGridPagerID, { edit: true, add: true, del: true, search: false },
        {
            ...
            //url: '/sic/listaaprobaciondocumento/' + parentRowKey + '/' + parentSolicitud,
