    ...
    $.fn.gantt = function (options) {
        ...
        var core = {
            ...
            leftPanel: function (element) {
                /* Left panel */
            	  var legends= "<div class='legends_div'>" +
            	  					"<div class='left_full'>" +
			            	  		"<span class='green_color'><span class='color_text'>Completed</span>" +
			            	  		"<span class='yellow_color'><span class='color_text'>Started</span>" +
			            	  		"<span class='red_color'><span class='color_text'>Overdue</span>" +
			            	  		"</div>" +
		            	  		"</div>";
            	  //  .append($('<div class="row spacer">'+legends+'</div>')
                ...
                $.each(element.data, function (i, entry) {
                	...
                	var labelString = entry.values[0].label;
                	var id = labelString.split('_')[0];
                	var newLabel = labelString.split('_')[1];
                	//console.log(entry.values[0].customClass);
                	/**
                	 * this is on click on task list for project detail page.
                	 */
                 	var task = entry.values[0].customClass.split('_')[0];
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        if(AllTrim(task)=="task"){
                        	entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '" id="row_' + id + '" onclick="getMilestoneDetails('+id+')"') + '>' + entry.name + '</span>');	
                        }else{
                        	entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '" id="row_' + id + '" onclick="getTaskDetails('+id+')"') + '>' + entry.name + '</span>');
                        }
            ...
            createProgressBar: function (days, cls, desc, label, dataObj) {
                ...
                var newLabel= label.split('_')[1];
                
                var bar = $('<div class="bar"><div class="fn-label"><span class="left display-none" >'+id+'</span>' + newLabel + '</div></div>')
                        .addClass(cls)
                        .css({
                ...
                bar.click(function (e) {
                    ...
                    settings.onItemClick( $(this).find("span").text());
                });
                return bar;
            },
