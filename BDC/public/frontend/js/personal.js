...
selectAno = document.getElementById('anoHistograma');
...
while (anoaingresar <= now.getFullYear()) {
	...
	anoaingresar = anoaingresar+1
}

now.setMonth(now.getMonth() - 1);
var num = 1
month.forEach(function (entry) {
	...
	$("#jqGridPersonal").jqGrid('navButtonAdd', '#jqGridPersonalPager', {
		...
		onClickButton: function () {
			...
			if (rowKey === null && typeof rowKey === "object") {
				...
				var now = new Date();
				var numeroMes = now.getMonth() + 1;
				var numeroAno = now.getFullYear();
				$("#jqGridHistograma").jqGrid('setGridParam', { url: '/listadoHistograma?mes=' + numeroMes + '&id=' + id + '&ano=' + numeroAno });
	...
	$("#jqGridHistograma").jqGrid({
		...
		loadComplete: function () {
			...
			$("#mesHistograma").change(function () {
				...
				var elano = $("#anoHistograma option:selected").val();
				...
				var id = rowData.id;
				//var id = 341;
				$grid.setGridParam({ caption: elmes, url: '/listadoHistograma?mes=' + elmes + '&id=' + id + '&ano=' + elano }).trigger("reloadGrid");
			});

			$("#anoHistograma").change(function () {
				var elmes = $("#mesHistograma option:selected").val();
				var elano = $("#anoHistograma option:selected").val();
	...
	$("#jqGridHistograma").jqGrid('navGrid', '#jqGridHistogramaPager', { refresh: false, add: false, edit: false, del: false, search: false })
		.jqGrid('navButtonAdd', '#jqGridHistogramaPager', {
			...
			onClickButton: function () {
				...
				var id = rowData.id;
				var url = 'histograma-excel?mes=' + elmes + '&id=' + id + '&ano=' + elano;
