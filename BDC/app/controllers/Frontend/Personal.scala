  ...
  def listadoAsignacion = Action {
    implicit request =>
      request.session.get("username").map { user =>
        ...
        val subtask = PersonalService.listadoAsignacionDependiente(uId, plan_start_date, plan_end_date, sidx, sord, rows, startRow)
        ...
        for (p <- subtask) {
          ...
          campo.put("avanzado", p.avanzado.getOrElse("").toString())
          registro.put(campo)
        }
