...
*/
  ...
  */
    ...
    request.session.get("username").map { user =>
      ...
      val pie = DashboardService.reportSap()

      var node = new JSONObject()

      node.put("showInLegend", false)
      node.put("titulo", "Programas con SAP")
      var puntos = new JSONArray()
      for (p <- pie) {
        var punto = new JSONObject()
        punto.put("dId", p.dId)
  ...
  def panelType() = Action { implicit request =>
    request.session.get("username").map { user =>
      val did = request.getQueryString("did").get.toString()
      val rows = request.getQueryString("rows").getOrElse("20").toString()
      val page = request.getQueryString("page").getOrElse("1").toString()
      ...
      if (!StringUtils.isEmpty(filters)) {
        ...
        if (!jObject.\\("rules").isEmpty) {
          ...
          if (tieneJson) {
            ...
            records = DashboardService.cantidadProgramaPorTipoFiltrado(did, qrystr)
            panel = DashboardService.reporteProgramaPorTipoFiltrado(did, rows, page, qrystr)

          } else {
            records = DashboardService.cantidadProgramaPorTipoFiltrado(did, "")
            panel = DashboardService.reporteProgramaPorTipoFiltrado(did, rows, page, "")
      ...
      if (!StringUtils.isEmpty(filters)) {
        ...
        if (!jObject.\\("rules").isEmpty) {
          ...
          if (tieneJson) {
            ...
            records = DashboardService.cantidadProgramaPorSubTipoFiltrado(did, qrystr)
            panel = DashboardService.reporteProgramaPorSubTipoFiltrado(did, rows, page, qrystr)

          } else {
            records = DashboardService.cantidadProgramaPorSubTipoFiltrado(did, "")
            panel = DashboardService.reporteProgramaPorSubTipoFiltrado(did, rows, page, "")
  ...
  def panelState() = Action { implicit request =>
    request.session.get("username").map { user =>
      val did = request.getQueryString("did").get.toString()
      val rows = request.getQueryString("rows").getOrElse("20").toString()
      val page = request.getQueryString("page").getOrElse("1").toString()
      ...
      if (!StringUtils.isEmpty(filters)) {
        ...
        if (!jObject.\\("rules").isEmpty) {
          ...
          if (tieneJson) {
            ...
            records = DashboardService.cantidadProgramaPorEstadoFiltrado(did, qrystr)
            panel = DashboardService.reporteProgramaPorEstadoFiltrado(did, rows, page, qrystr)

          } else {
            records = DashboardService.cantidadProgramaPorEstadoFiltrado(did, "")
            panel = DashboardService.reporteProgramaPorEstadoFiltrado(did, rows, page, "")
...
//
  def panelSap() = Action { implicit request =>
    request.session.get("username").map { user =>
      val did = request.getQueryString("did").get.toString()
      val rows = request.getQueryString("rows").getOrElse("20").toString()
      val page = request.getQueryString("page").getOrElse("1").toString()
      ...
      if (!StringUtils.isEmpty(filters)) {
        ...
        if (!jObject.\\("rules").isEmpty) {
          ...
          if (tieneJson) {
            ...
            records = DashboardService.cantidadProgramaPorSapFiltrado(did, qrystr)
            panel = DashboardService.reporteProgramaPorSapFiltrado(did, rows, page, qrystr)

          } else {
            records = DashboardService.cantidadProgramaPorSapFiltrado(did, "")
            panel = DashboardService.reporteProgramaPorSapFiltrado(did, rows, page, "")
  ...
  def reportFull2 = Action {
    implicit request =>
      request.session.get("username").map { user =>
        ...
        val file = new File("reportefabrica.xlsx")
        val fileOut = new FileOutputStream(file);
        val wb = new XSSFWorkbook
        val sheet = wb.createSheet("PROGRAMA")
        var j = 0
        ...
        val panel = DashboardService.createReportFabricas()
        ...
        wb.write(fileOut);
        fileOut.close();
        Ok.sendFile(content = file, fileName = _ => "reportefabrica.xlsx")
      }.getOrElse {
        Redirect(routes.Login.loginUser()).withNewSession
      }
  }
