...
object SubTaskJira extends Controller {
  ...
  def newSubTask(id: String) = Action { implicit request =>   
      ...
      val taskDetails = TaskService.findTaskDetailsByTaskId(Integer.parseInt(id))
  ...
  def saveSubTask(id: String) = Action { implicit request =>
      ...
      ARTForms.subTaskForm.bindFromRequest.fold(
        hasErrors => {
          ...
          val taskDetails = TaskService.findTaskDetailsByTaskId(_id)
          val projectDetails = ProjectService.findProject(taskDetails.get.pId)
          val programDetails = ProgramService.findProgramMasterDetailsById(projectDetails.get.program.toString)
          
          BadRequest("Error: "+ hasErrors)
        },
        success => {
          val id = Integer.parseInt(success.task_id.toString())
          val taskDetails = TaskService.findTaskDetailsByTaskId(id)
          val projectDetails = ProjectService.findProject(taskDetails.get.pId)

          val theForm = SubTaskServices.validateForm(ARTForms.subTaskForm.fill(success), taskDetails, "")
          if (theForm.hasErrors) {
            val programDetails = ProgramService.findProgramMasterDetailsById(projectDetails.get.program.toString)
            var comp_per_task: Double = 0
            if (!taskDetails.get.completion_percentage.isEmpty) {
              comp_per_task = taskDetails.get.completion_percentage.get.toDouble
            }
             ...
             * Activity log
             */
            val act = Activity(ActivityTypes.SubTask.id, "New sub task created by Jira ", new Date(), 1202, lastId.toInt)
  ...
  def updateSubTask(id: String) = Action { implicit request =>
      ...
      ARTForms.subTaskForm.bindFromRequest.fold(
        ...
        success => {
          ...
          val theForm = SubTaskServices.validateForm(ARTForms.subTaskForm.fill(success), taskDetails, id)
          if (theForm.hasErrors) {
            val subTask = SubTaskServices.findSubTaskDetailsBySubtaskId(id)
            val taskDetails = TaskService.findTaskDetailsByTaskId(subTask.get.task_id)
            val projectDetails = ProjectService.findProject(taskDetails.get.pId)
            ...
            if (dates_changed) {
              val baseline = Baseline(None, changeState.toString(), 1202, new Date(), "subtask", success.sub_task_id.get.toInt);
             ...
             * Activity log
             */
            val act = Activity(ActivityTypes.SubTask.id, "Sub task updated by Jira " , new Date(), 1202, Integer.parseInt(id))
            Activity.saveLog(act)
            TaskService.updateTaskStatus(subtask.get.sub_task_id.get.toString())
            Ok("Tarea "+Integer.parseInt(id)+" modificada")
          }
  ...
  def saveSubTaskAllocation() = Action { implicit request =>
    var node = new JSONObject()
      ...
      if (StringUtils.equals(mode, "add")) {
         ...
         * Activity log
         */
        val act = Activity(ActivityTypes.Allocation.id, "Sub task allocated by Jira ", new Date(), 1202, last.toInt)
        Activity.saveLog(act)
  ...
  def saveSubTaskAllocationExternal() = Action { implicit request =>
      ...
      if (StringUtils.equals(mode, "add")) {
         ...
         * Activity log
         */
        val act = Activity(ActivityTypes.Allocation.id, "Sub task external allocated by Jira " , new Date(), 1202, last.toInt)
        Activity.saveLog(act)
  ...
  def deleteSubtask(id: String) = Action { implicit request =>
     ...
     * Activity log
     */
    val act = Activity(ActivityTypes.SubTask.id, "Sub task deleted by Jira ", new Date(), 1202, Integer.parseInt(id))
    Activity.saveLog(act)
