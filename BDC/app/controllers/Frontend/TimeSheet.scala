...
object TimeSheet extends Controller {
  ...
  def saveHoursForSubtask(_date: String) = Action { implicit request =>
    request.session.get("username").map { user =>
      val user_id = Integer.parseInt(request.session.get("uId").get.toString())
      val utype = Integer.parseInt(request.session.get("utype").get.toString())
      ...
      request.body.asJson.map { jjjson =>
        ...
        for (subtask <- subtasks) {
          ...
          if (!min.isEmpty()) {
            minutes = Math.ceil(min.toInt * 100 / 60).toInt
          }
          println("min    " + minutes)
          println("nin divi 100       " + (minutes / 100).toDouble)

          var hours = (hr.toInt + minutes / 100).toDouble

          //   val hheee: Double = hr.toDouble + minutes / 100

          println(minutes + "minutes formatted")
          println(hours + "====hours")
          println(hr + "====hr")*/
   ...
   */
  def daleteTimesheet() = Action { implicit request =>
    request.session.get("username").map { user =>
      val tid = request.getQueryString("tid").get
      TimesheetService.deleteTimesheet(Integer.parseInt(tid.toString()))
      /**
       * Activity log
       */
      val act = Activity(ActivityTypes.Timesheet.id, "Timesheet entry deleted by " + request.session.get("username").get, new Date(), Integer.parseInt(request.session.get("uId").get), Integer.parseInt(tid))
      Activity.saveLog(act)
