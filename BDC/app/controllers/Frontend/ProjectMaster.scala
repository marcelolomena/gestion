...
object ProjectMaster extends Controller {
  ...
  def addNewProject(program: String) = Action { implicit request =>
    request.session.get("username").map { user =>
      ...
      */
      ...
      Ok(views.html.frontend.project.addProject(pIndex, ARTForms.projectForm, pmMap, pTypes, workFlow)).withSession("username" -> request.session.get("username").get, "utype" -> request.session.get("utype").get, "uId" -> request.session.get("uId").get, "user_profile" -> request.session.get("user_profile").get)
    }.getOrElse {
      Redirect(routes.Login.loginUser())
    }
  }
  ...
  def saveProjectDetails(program_id: String) = Action { implicit request =>
    ...
    oldfrom.fold(
      errors => {
        ...
        BadRequest(views.html.frontend.project.addProject(Integer.parseInt(errors.data.get("program").get), the_Form, pmMap, pTypes, workFlow)).withSession("username" -> request.session.get("username").get, "utype" -> request.session.get("utype").get, "uId" -> request.session.get("uId").get, "user_profile" -> request.session.get("user_profile").get)
      },
      ProjectMaster => {
        ...
        if (the_Form.hasErrors) {
          BadRequest(views.html.frontend.project.addProject(Integer.parseInt(the_Form.data.get("program").get), the_Form, pmMap, pTypes, workFlow)).withSession("username" -> request.session.get("username").get, "utype" -> request.session.get("utype").get, "uId" -> request.session.get("uId").get, "user_profile" -> request.session.get("user_profile").get)
        } else {
           ...
           * Activity log
           */
          val act = Activity(ActivityTypes.Project.id, "New project created by " + request.session.get("username").get, new Date(), Integer.parseInt(request.session.get("uId").get), pId.toInt)
