...
object TaskJira extends Controller {
  ...
  def projectTaskDetails(task_id: String) = Action { implicit request =>
      ...
      if (!taskDetail.isEmpty) {
        respuesta.put( play.api.libs.json.Json.toJson(taskDetail))
        val subtasks = TaskService.findSubTaskListByTaskId(task_id)
        respuesta.put(play.api.libs.json.Json.toJson(subtasks))
      ...
      success => {
        ...
        if (theForm.hasErrors) {
          ...
          val milestoneDetails = Tasks(None, success.project, success.task_title, success.task_details.task_code,
            ...
            new Date(), success.task_status, 1202, success.owner, Option(success.task_discipline), success.completion_percentage,
           ...
           * Activity log
           */
          val act = Activity(ActivityTypes.Task.id, "New task created by Jira ", new Date(), 1202, latest_task.toInt)
          Activity.saveLog(act)

         Ok(latest_task.toString())
        }
      })
  }
    ...
    ARTForms.taskForm.bindFromRequest.fold(
      ...
      success => {
        ...
        if (theForm.hasErrors) {
           ...
           * Activity log
           */
          val act = Activity(ActivityTypes.Task.id, "Task updated by Jira" , new Date(), 1202, Integer.parseInt(id))
          Activity.saveLog(act)

          /**
           * Update Baseline...
           */
          TaskService.taskBasline(task, success.plan_start_date, success.plan_end_date, "1202")

          Ok("Tarea "+id+" modificada")
        }
      })
  }
...
*/
  ...
  def deleteTask(task_id: String) = Action { implicit request =>
      ...
      if (do_delete) {
         ...
         * Activity log
         */
        val act = Activity(ActivityTypes.Task.id, "Task deleted by Jira ", new Date(), 1202, task_id.toInt)
        Activity.saveLog(act)
